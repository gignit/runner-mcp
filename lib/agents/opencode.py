...
def looks_like_session_id(s: str) -> bool:
    """opencode session ids look like `ses_...`. Shape check only; there
    is no cheap local existence check, so adoption is unvalidated and a
    bad id surfaces as a 'Session not found' error in stderr.log."""
    return bool(s) and s.startswith("ses_")


def list_models(provider: str) -> list[str]:
    """Return the valid `provider/model` identifiers for one provider.

    Shells out to `opencode models <provider>`, which prints one
    `provider/model` per line. Returns [] when opencode is missing, the
    provider is unknown, or the command errors -- callers decide how to
    treat an empty list (validate_model surfaces it as "no models found").
    """
    if not provider:
        return []
    try:
...
# Shown by the `models` CLI command next to this backend's list. Unlike the
# claude backend, opencode's catalog is authoritative: validate_model refuses
# anything not in it.
MODELS_NOTE = (
    "Authoritative catalog from `opencode models`; a --model value must "
    "match one of these provider/model ids exactly."
)


def available_models() -> list[dict[str, Any]]:
    """All models this backend accepts for --model, for the `models`
    CLI command. Shells out to `opencode models` (no provider arg =
    every provider). Returns [{"model": "provider/model"}, ...]; empty
    when opencode is missing or errors.
    """
    try:
        proc = subprocess.run(
            ["opencode", "models"],
...
def validate_model(model: str) -> tuple[bool, list[str]]:
    """Check `model` against opencode's real model catalog for its provider.

    `model` is expected in `provider/model` form (e.g.
    "anthropic/claude-fable-5"). The provider (segment before the first
    "/") drives which catalog we query: `opencode models <provider>`.

    Returns (is_valid, valid_models). valid_models is the full list for
    the provider so the caller can render it in the invalid-model error.
    A malformed model (no "/") is invalid with an empty list.
    """
    if not model or "/" not in model:
        return (False, [])
    provider = model.split("/", 1)[0]
    valid = list_models(provider)
...
def build_cmd(prompt_path: Path, session_id: str | None, model: str | None = None) -> str:
    ...
    Pipeline:
        opencode run --format json [-m MODEL] [-s SID] < prompt.md | python3 <renderer>

    `model` is an optional `provider/model` override (e.g.
    "anthropic/claude-fable-5"). When omitted, opencode uses its own
    configured default -- the caller is expected to validate the model
    (see validate_model) BEFORE building the command; build_cmd trusts
    ...
    model_flag = f"-m {_sh_quote(model)} " if model else ""
    return (
        f"opencode run --format json {model_flag}{session_flag}"
