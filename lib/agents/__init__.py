...
Optionally a runtime module may also expose:

    available_models() -> list[dict]
        [{"model": id, "description"?: str}, ...] -- what the `models`
        CLI command lists for this backend. Omit when the backend has
        no way to enumerate models.

    MODELS_NOTE: str
        One-liner shown next to the list saying how binding it is
        (authoritative catalog vs advisory pass-through).

    validate_model(model) -> tuple[bool, list[str]]
        Pre-spawn check of a --model override; absent = accept anything.

    looks_like_session_id(s) -> bool
        Shape claim used by session adoption (opencode 'ses_...',
        claude/codex UUID). Absent = the runtime never claims an id, except
        when --agent selected it explicitly.

    find_session(session_id, cwd) -> dict | None
        On-disk verification for adoption. None = no such session.
        Otherwise {"projectDir": str|None, "sameProject": bool, ...};
        when the session belongs to another project, runner spawns the
        backend in projectDir so resumed sessions retain their original
        cwd while the run dir stays in the active project's run root.
        Absent = adoption is unvalidated (opencode).

Adding a new agent backend (e.g. 'codex') = one new module in this
