...
type model struct {
	...
	// listInFlight guards against stacking `runner list` subprocesses.
	// A list can take longer than one tick (it shells out to python3 and
	// scans every run dir), so ticks must not spawn a new one until the
	// previous listMsg lands -- otherwise processes pile up without bound.
	listInFlight bool
...
func newModel(scopeDir, corePath string) model {
	...
	m := model{
		...
		listInFlight:     true, // Init issues the first loadJobsCmd
		selectedIdx:      -1,
		autoMode:         true,
...
// logTailCapBytes bounds how much of each log the TUI loads per refresh.
// Logs are re-read every tick while a run is selected; reading a runaway
// multi-GB log in full would stall the UI and balloon memory.
const logTailCapBytes = 2 << 20

// readLogTail returns up to the last logTailCapBytes of the file. When the
// file is larger, the partial first line is dropped and a truncation banner
// is prepended so the user knows they're looking at a tail.
func readLogTail(path string) (string, error) {
	...
	size := st.Size()
	if size > logTailCapBytes {
		if _, err := f.Seek(size-logTailCapBytes, io.SeekStart); err != nil {
			return "", err
		}
	}
	...
	if size > logTailCapBytes {
		...
		return fmt.Sprintf("[showing last %d KB of %d KB]\n%s", len(data)/1024, size/1024, data), nil
	}
...
func loadLogsCmd(runID, runRoot string) tea.Cmd {
	return func() tea.Msg {
		...
		stdout, err := readLogTail(stdoutPath)
		if err != nil && !errors.Is(err, os.ErrNotExist) {
			return logsMsg{runID: runID, err: err}
		}
		stderr, err := readLogTail(stderrPath)
		if err != nil && !errors.Is(err, os.ErrNotExist) {
			return logsMsg{runID: runID, err: err}
		}
