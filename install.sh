...
#      (opencode, Claude Code, Codex, Grok, VS Code / Cursor), repairing any
#      existing registration whose server script no longer exists (e.g. one
#      left behind by an older install layout).
#
...
# Registration policy: ADD IF MISSING, REPAIR IF DEAD. If a `runner` entry
# already exists in an agent's config AND still launches a server script that
# exists on disk, we treat it as USER-OWNED and never modify it -- the user
# may have manually tuned options (timeout, env, custom command). But an entry
# whose script is GONE (e.g. left behind by an older install layout such as
# ~/.runner) can never work, so it is removed and re-registered instead of
# being "left unchanged" forever. New entries launch the server with the
# absolute node path ($NODE_BIN) so GUI-launched agents find node reliably.

# Is $1 an absolute path that no longer exists? Relative or empty paths can't
# be judged from here, so they count as healthy and the entry is left alone.
script_missing() {
  case "$1" in
    /*) [ ! -e "$1" ] ;;
    *)  return 1 ;;
...
# Pull the first absolute *.js path out of an agent CLI's "show entry" output
# -- the server script the registration launches.
entry_script_from_output() {
  grep -oE '/[^[:space:]",]+\.js' 2>/dev/null | head -n1
}

# jq expression: given a registration bound to $r, the script it launches
# (last arg if any, else the bare command).
JQ_ENTRY_SCRIPT='if $r == null then "" elif (($r.args // []) | length) > 0 then $r.args[-1] else ($r.command // "") end'
...
register_opencode() {
  ...
  if jq -e '.mcp.runner != null' "$cfg" >/dev/null 2>&1; then
    script="$(jq -r '(.mcp.runner.command // [])[-1] // empty' "$cfg" 2>/dev/null)"
    if script_missing "$script"; then
      tmp="$cfg.tmp.$$"
      jq 'del(.mcp.runner)' "$cfg" > "$tmp" && mv "$tmp" "$cfg"
      warn "opencode runner entry launched missing $script -- re-registering"
    else
...
# Claude Code. Scopes matter here: an entry can live at user scope
# (~/.claude.json .mcpServers -- applies in every project) or local scope
# (~/.claude.json .projects["<dir>"].mcpServers -- one directory only). We
# register at USER scope so the server works everywhere; a bare
# `claude mcp add` would only cover the directory the installer ran in.
# With jq we also repair the config first: dead entries are removed at both
# scopes, and local-scope entries that just launch our installed server with
# no customization are dropped as redundant -- they shadow the user-scope
# entry and trip Claude's conflicting-scope diagnostic.
register_claude() {
  need claude || return 0
  cfg="$HOME/.claude.json"

  if ! need jq || [ ! -f "$cfg" ]; then
    # No jq: we can only see the entry that wins for the current directory.
    if claude mcp get runner >/dev/null 2>&1; then
      script="$(claude mcp get runner 2>/dev/null | entry_script_from_output)"
      if script_missing "$script"; then
        warn "Claude Code has a runner entry launching missing $script -- install jq and re-run, or fix by hand:"
        warn "    claude mcp remove runner && claude mcp add runner -s user -- \"$NODE_BIN\" \"$MCP_ENTRY\""
      else
        ok "Claude Code already has a runner entry -- left unchanged"
      fi
      return 0
    fi
    claude mcp add runner -s user -- "$NODE_BIN" "$MCP_ENTRY" >/dev/null 2>&1 \
      && ok "Registered with Claude Code at user scope (claude mcp add -s user)" \
      || warn "claude found but registration failed; run: claude mcp add runner -s user -- \"$NODE_BIN\" \"$MCP_ENTRY\""
    return 0
  fi

  # 1. Repair a dead user-scope entry.
  script="$(jq -r ".mcpServers.runner as \$r | $JQ_ENTRY_SCRIPT" "$cfg" 2>/dev/null)"
  if script_missing "$script"; then
    claude mcp remove runner -s user >/dev/null 2>&1 \
      && warn "Claude Code user-scope runner entry launched missing $script -- removed"
  fi

  # 2. Ensure a healthy user-scope entry exists.
  if jq -e '.mcpServers.runner != null' "$cfg" >/dev/null 2>&1; then
    ok "Claude Code already has a user-scope runner entry -- left unchanged"
  else
    claude mcp add runner -s user -- "$NODE_BIN" "$MCP_ENTRY" >/dev/null 2>&1 \
      && ok "Registered with Claude Code at user scope (claude mcp add -s user)" \
      || warn "claude found but registration failed; run: claude mcp add runner -s user -- \"$NODE_BIN\" \"$MCP_ENTRY\""
  ...
  # 3. Sweep per-project local-scope entries: dead ones always go; entries
  #    that merely launch our installed server via node with no env go too
  #    (redundant once user scope covers them). Anything customized stays.
  have_user=""
  jq -e '.mcpServers.runner != null' "$cfg" >/dev/null 2>&1 && have_user=1
  jq -r '.projects // {} | to_entries[] | select(.value.mcpServers.runner != null) | .key' "$cfg" 2>/dev/null \
  | while IFS= read -r proj; do
      [ -d "$proj" ] || continue   # `claude mcp remove -s local` must run inside the project
      pscript="$(jq -r --arg p "$proj" ".projects[\$p].mcpServers.runner as \$r | $JQ_ENTRY_SCRIPT" "$cfg" 2>/dev/null)"
      if script_missing "$pscript"; then
        ( cd "$proj" && claude mcp remove runner -s local >/dev/null 2>&1 ) \
          && warn "Claude Code local-scope runner entry in $proj launched missing $pscript -- removed"
      elif [ -n "$have_user" ] && jq -e --arg p "$proj" --arg e "$MCP_ENTRY" \
             '.projects[$p].mcpServers.runner as $r | ($r.args // []) == [$e] and (($r.env // {}) | length) == 0 and (($r.command // "") | test("(^|/)node$"))' \
             "$cfg" >/dev/null 2>&1; then
        ( cd "$proj" && claude mcp remove runner -s local >/dev/null 2>&1 ) \
          && ok "Claude Code local-scope runner entry in $proj is covered by user scope -- removed"
      fi
    done
}

# Codex: `codex mcp get` exits 0 if a runner entry exists; its output shows
# the launch command, which lets us detect (and repair) a dead entry.
register_codex() {
  need codex || return 0
  if codex mcp get runner >/dev/null 2>&1; then
    script="$(codex mcp get runner 2>/dev/null | entry_script_from_output)"
    if script_missing "$script"; then
      codex mcp remove runner >/dev/null 2>&1
      warn "Codex runner entry launched missing $script -- re-registering"
    else
...
# Grok: `grok mcp list` prints one "  name: <command...>" line per entry,
# which lets us both detect an existing runner entry and repair a dead one.
register_grok() {
  need grok || return 0
  line="$(grok mcp list 2>/dev/null | grep -E '(^|[[:space:]])runner:([[:space:]]|$)' | head -n1)"
  if [ -n "$line" ]; then
    script="$(printf '%s\n' "$line" | entry_script_from_output)"
    if script_missing "$script"; then
      grok mcp remove runner >/dev/null 2>&1
      warn "Grok runner entry launched missing $script -- re-registering"
    else
...
register_vscode() {
  ...
  for variant in "VS Code:Code" "VS Code Insiders:Code - Insiders" "Cursor:Cursor"; do
    ...
    if jq -e '.servers.runner != null' "$cfg" >/dev/null 2>&1; then
      script="$(jq -r ".servers.runner as \$r | $JQ_ENTRY_SCRIPT" "$cfg" 2>/dev/null)"
      if script_missing "$script"; then
        tmp="$cfg.tmp.$$"
        jq 'del(.servers.runner)' "$cfg" > "$tmp" && mv "$tmp" "$cfg"
        warn "$label runner entry launched missing $script -- re-registering"
      else
...
uninstall() {
  ...
  unregister_opencode
  unregister_via_cli "Claude Code (user scope)"  claude mcp remove runner -s user
  unregister_via_cli "Claude Code (local scope)" claude mcp remove runner -s local
