...
# --omit=optional: @anthropic-ai/claude-agent-sdk's optionalDependencies are
# ~100MB platform-specific vendored claude binaries. The payload ships
# mcp/node_modules to EVERY platform, so those must never be installed --
# lib/agents/claude_runner.mjs uses the user's own `claude` executable instead.
build-mcp:
	@echo "> Building MCP server (TypeScript -> mcp/dist)"
	@cd "$(MCP_DIR)" && npm install --omit=optional --silent && npm run build --silent
