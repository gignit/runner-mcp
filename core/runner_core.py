...
FILE_LIST_CACHE = "list_cache.json"  # cached scoreboard entry for terminal runs
...
# Agent-turn stall threshold. A sub-agent run whose process is still ALIVE
# but has produced no stdout for this long is almost certainly a dead
# opencode stream (the turn ended mid-output without a reason=stop / error
# event, leaving the process idle in its event loop). cmd_list/synthesize
# surface such runs with stalled=true + idleSec so they don't masquerade as
# healthy "running" jobs. This never auto-kills -- the operator/agent decides.
AGENT_STALL_SEC = 300
...
# Normalized test-summary shaping for adapter runs (e.g. go-test).
#
# The philosophy: a test status response should be proportional to the
# FAILURES, not to the size of the suite. A green run collapses to a
# compact summary; a red run inlines each failure's assertion detail plus
# a precise pointer (logRef) into stdout.log for drill-down. The raw
# per-package/per-test event stream stays on disk (runner_section /
# runner_grep) and is suppressed from the default status response for
# adapter runs -- it just re-states, package by package, what the summary
# already carries.
#
# TEST_FAILURES_INLINE_CAP: max failing tests inlined into
#   testSummary.failures. Beyond this, failuresTruncated + nextCalls point
#   the agent at runner_section. Prevents a build-break "500 tests failed"
#   run from becoming the context bomb the summary exists to avoid.
# TEST_FAILURE_DETAIL_CAP: max assertion-detail lines inlined per failing
#   test. Beyond this the logRef covers the rest.
# TEST_SLOWEST_PACKAGES: how many slowest packages to surface on a green
#   run (the one "everything passed but here's something" signal agents
#   act on). Omitted entirely when empty.
TEST_FAILURES_INLINE_CAP = 10
TEST_FAILURE_DETAIL_CAP = 15
TEST_SLOWEST_PACKAGES = 3
...
def analyze_command(cmd: str) -> dict[str, Any]:
    ...
    # Record the individual steps and which chain operators actually appear, so
    # the gate message can NAME the operator and show the exact split into
    # separate runner_start calls (the #1 request from agents who kept
    # re-learning the rule).
    result["logicSteps"] = [p.strip() for p in logic_parts]
    ops_present: list[str] = []
    for op in ("&&", "||", ";"):
        # detect at top level only (reuse split_top: if splitting on just this
        # operator yields >1 part, it is present outside quotes)
        two = {op} if op in ("&&", "||") else set()
        one = {op} if op == ";" else set()
        if len(split_top(cmd, two, one)) > 1:
            ops_present.append(op)
    result["logicOps"] = ops_present
...
def _build_command_gate(cmd: str, analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    if filters and not multi:
        ...
        ops = analysis.get("logicOps") or []
        steps = analysis.get("logicSteps") or []
        # Name the offending operator(s) explicitly up front -- agents reported
        # re-learning the rule because the message never said WHICH operator or
        # HOW to split. e.g. "chain operator '&&' detected".
        op_label = (
            f"chain operator {ops[0]!r} detected"
            if len(ops) == 1
            else "chain operators " + ", ".join(f"{o!r}" for o in ops) + " detected"
        )
        why = (
            f"{op_label}: your command joins {analysis.get('logicCount')} steps "
            f"with && / || / ; . As one line you lose per-step visibility (which "
            f"step failed, how long each took) and must escape quotes/pipes across "
            f"the whole thing. Pick ONE of the two fixes below."
        )
        fix = (
            "Split the chain -- run each step as its OWN runner_start "
            "(recommended for a few steps), OR put the steps in a reusable script "
            "and run that (best for repeated/complex work). Both give you "
            "structured per-step status for free."
        )
    ...
    # For a chain, show the EXACT split into separate runner_start calls as the
    # first, most direct option -- so an agent can act without re-deriving it.
    if multi:
        steps = analysis.get("logicSteps") or []
        payload["optionA_splitIntoSeparateRunnerStarts"] = {
            "note": (
                "Run each step as its own runner_start, in order. Best when the "
                "steps are few and you want per-step status. Note: separate runs "
                "do NOT share shell state -- if a later step depends on a `cd` or "
                "an env var set by an earlier one, fold that into the step's own "
                "cmd (or use cwd=), or use option B."
            ),
            "calls": [
                {"tool": "runner_start", "cmd": s} for s in steps
            ],
        }
    payload["optionB_scriptWorkflow"] = {
            "note": (
    ...
    payload["escapeHatch"] = (
        "If this chain is truly one atomic step that MUST share shell state "
        "(e.g. `cd x && ./run`) and neither option above fits, re-send with "
        "noScrub: true to run the exact string verbatim."
    )
    payload["yourCommand"] = cmd
    return payload
...
# -----------------------------------------------------------------------------
# Log line normalization (CENTRAL -- applied once, at every read)
# -----------------------------------------------------------------------------
#
# Storage stays RAW: stdout.log / stderr.log keep the exact bytes the process
# emitted (ANSI colors and all) for "hop the fence" forensics. But every
# AGENT-FACING surface -- parse_events (adapters + testSummary), runner_grep,
# runner_status stdout/stderr, runner_section tails, the warnings scan -- reads
# lines through _read_log_line() below, which strips terminal escape sequences.
#
# Doing it here, at the single point a stored line becomes in-memory text,
# means nothing downstream ever has to think about ANSI again: patterns match
# the VISIBLE text (so `foo_test.go:\d+` matches even when color codes sat
# between the filename and the line number), and responses aren't littered with
# \x1b[..m noise. Stripping is per-line and never changes line COUNT, so every
# lineNo / logRef range / grep bound stays exactly valid.
#
# The regex covers the common SGR color/style codes plus the general CSI form;
# it is deliberately conservative (only well-formed escape sequences) so it
# can never corrupt real log text.
_ANSI_ESCAPE_RE = re.compile(r"\x1b(?:\[[0-9;?]*[ -/]*[@-~]|\][^\x07\x1b]*(?:\x07|\x1b\\)|[@-Z\\-_])")


def _strip_ansi(s: str) -> str:
    """Remove terminal escape sequences from a single line of log text."""
    return _ANSI_ESCAPE_RE.sub("", s)


def _read_log_line(line: str) -> str:
    """Normalize ONE raw stored log line for agent-facing presentation:
    drop the trailing newline and strip terminal escape sequences. Every
    reader funnels through this so ANSI handling lives in exactly one place.
    """
...
def parse_events(stdout_path: Path, parser_hint: str = "auto") -> list[ParsedEvent]:
    ...
    # Stream the file: buffer only the first ADAPTER_SNIFF_LINES lines to
    # decide marker-vs-adapter mode, then walk the rest line by line. The
    # full log is never materialized in memory at once (list runs over
    # large histories would otherwise load every run's whole stdout.log).
    with stdout_path.open("r", encoding="utf-8", errors="replace") as f:
        head: list[str] = []
        for line in f:
            head.append(line)
            if len(head) >= ADAPTER_SNIFF_LINES:
                break

        # Decide whether to engage an adapter
        has_markers = any(l.startswith(WIRE_PREFIX) for l in head)
        adapter: OutputAdapter | None = None
        if parser_hint != "none":
            if parser_hint == "auto":
                if not has_markers:
                    adapter = _select_adapter([_read_log_line(l) for l in head])
            elif parser_hint:
                adapter = _ADAPTERS_BY_NAME.get(parser_hint)
                if adapter is not None:
                    adapter = adapter.__class__()  # fresh instance for stateful parsing

        # Walk lines (sniffed head first, then the rest of the stream),
        # dispatch to marker parser or adapter. Every line is normalized once
        # here (ANSI stripped, newline trimmed) so adapters + everything
        # derived from them -- testSummary, failure detail, msgs -- are clean
        # without each call site stripping. `::run::` markers are plain ASCII,
        # so normalization never affects marker detection or payloads.
        for line_no, raw_line in enumerate(itertools.chain(head, f), start=1):
            line = _read_log_line(raw_line)
            if line.startswith(WIRE_PREFIX):
                payload_str = line[len(WIRE_PREFIX):]
                try:
                ...
                for ev in adapter.feed(line, line_no, file_mtime):
...
# `=== NAME`, `=== CONT`, `=== PAUSE` all re-announce which test owns the
# output that follows. With parallel tests (t.Parallel) go batches each
# test's buffered output under a `=== NAME  TestX` marker emitted right
# before that test's assertion lines -- long after its `=== RUN`. Tracking
# these is what lets us attribute a failure-detail block to the correct
# test instead of whatever ran last.
_GO_TEST_NAME_RE = re.compile(r"^=== (?:NAME|CONT|PAUSE)\s+(\S+)")
...
# Header line that opens an assertion block: "    foo_test.go:42: message".
_GO_TEST_FAIL_LINE_RE = re.compile(r"^\s*\S+\.go:\d+:")
# Indented continuation lines of a testify-style assertion block, e.g.
#   "        \tError Trace:\t/path/foo_test.go:216"
#   "        \tError:      \tAn error is expected but got nil."
#   "        \tTest:       \tTestX"
#   "        \tMessages:   \t..."
# These carry the actual assertion MESSAGE (the part an agent needs to fix
# the bug), and go emits them indented under the header line. We treat any
# line indented past the header's column as part of the current block.
_GO_TEST_DETAIL_CONT_RE = re.compile(r"^\s+\S")
# A panic dump opens with "panic:" (or "--- FAIL" then "panic:"); the
# goroutine trace that follows is captured as detail on the owning test.
_GO_TEST_PANIC_RE = re.compile(r"^\s*(panic:|fatal error:|goroutine\s+\d+)")


def _clean_go_detail_line(text: str) -> str:
    """Normalize a captured go-test failure-detail line into compact,
    agent-readable text.

    go/testify indents assertion blocks and uses tab columns:
        "        \\tError Trace:\\t/path/foo_test.go:216"
        "        \\tError:      \\tAn error is expected but got nil."
        "        \\tTest:       \\tTestX"
    We collapse the leading indentation + internal tab runs into single
    spaces and squeeze the "Label:   value" columns to "Label: value" so a
    failure like "Error: An error is expected but got nil." reads cleanly in
    testSummary.failures[].detail without the raw whitespace noise.
    """
    # Replace tabs with spaces, strip outer whitespace, collapse runs.
...
class GoTestAdapter(OutputAdapter):
    ...
    def __init__(self) -> None:
        ...
        # The test that owns the output currently being emitted. Set by
        # `=== RUN` and re-set by `=== NAME`/`=== CONT` (parallel tests batch
        # their buffered output under a NAME marker long after RUN).
        self._current_test: str | None = None
        # Failure-detail lines accumulated PER TEST, keyed by test name, as we
        # see them -- go emits the assertion block (header + indented
        # continuation lines) under the owning test's NAME marker, which for
        # parallel tests precedes the batched `--- FAIL` lines. Attaching by
        # current-test name (not a single rolling buffer) is what keeps each
        # failing test's detail correct when several fail in one package.
        self._fail_details: dict[str, list[tuple[int, str]]] = {}
        # True while we're inside an assertion/panic block for _current_test,
        # so indented continuation lines get appended to that test's detail.
        self._in_detail_block = False
    ...
    def feed(self, line: str, line_no: int, fallback_at: int) -> list[ParsedEvent]:
        ...
        # `=== NAME`/`=== CONT`/`=== PAUSE`: re-point current-test to the
        # owner of the output that follows. Critical for parallel tests,
        # whose assertion blocks are batched under NAME markers well after
        # their RUN line and before the grouped `--- FAIL` results.
        m = _GO_TEST_NAME_RE.match(line)
        if m:
            self._current_test = m.group(1)
            self._in_detail_block = False
            return events
        ...
        # Assertion-block capture. Detail is accrued PER TEST, keyed by the
        # current test name, as we encounter it (the `--- FAIL` line just
        # records the result -- detail is already stored by then).
        #
        # A block opens on a `file.go:NN:` header line and continues through
        # the indented continuation lines that follow (testify's
        # "Error Trace: / Error: / Test: / Messages:" block, or any indented
        # message). A non-indented line ends the block.
        if self._current_test:
            if _GO_TEST_FAIL_LINE_RE.match(line):
                self._fail_details.setdefault(self._current_test, []).append(
                    (line_no, line.rstrip())
                )
                self._in_detail_block = True
                return events
            if self._in_detail_block and _GO_TEST_DETAIL_CONT_RE.match(line):
                # Indented continuation of the current assertion block.
                self._fail_details.setdefault(self._current_test, []).append(
                    (line_no, line.rstrip())
                )
                return events
            if _GO_TEST_PANIC_RE.match(line):
                # Panic / fatal dump: capture the opening line and the
                # goroutine trace that follows (handled as continuation).
                self._fail_details.setdefault(self._current_test, []).append(
                    (line_no, line.rstrip())
                )
                self._in_detail_block = True
                return events
            # Any other non-indented line ends an open detail block.
            if self._in_detail_block and not _GO_TEST_DETAIL_CONT_RE.match(line):
                self._in_detail_block = False
        ...
        for t in self._tests:
            ...
            if t["status"] == "fail":
                for cap_line_no, text in self._fail_details.get(t["name"], []):
                    cleaned = _clean_go_detail_line(text)
                    if not cleaned:
                        continue
                    out.append(ParsedEvent(
                        line_no=cap_line_no,
                        ...
                        kind="event",
                        section=pkg,
                        payload={"v": "event", "msg": cleaned, "kind": "failure-detail", "test": t["name"]},
                    ))
...
# Verbose per-test line: "path/test.py::test_name PASSED [ 20%]"
# (also FAILED / ERROR / SKIPPED / XFAIL / XPASS). The nodeid may include
# "::Class::method" and parametrization "[case]".
_PYTEST_VERBOSE_RE = re.compile(
    r"^(\S+\.py::\S+?)\s+(PASSED|FAILED|ERROR|SKIPPED|XFAIL|XPASS)\b"
)
# short-test-summary-info entry: "FAILED path::test - AssertionError: ..."
# The reason after " - " is the single best one-line failure detail.
_PYTEST_SUMMARY_LINE_RE = re.compile(
    r"^(FAILED|ERROR)\s+(\S+\.py::\S+?)(?:\s+-\s+(.*))?$"
)
# Final result line. Two forms:
#   banner (-v/default): "===== 1 failed, 10 passed in 145.68s ====="
#   bare (-q):           "1 passed in 1.27s"
# Capture the inner text (the count clauses + "in <dur>s") for both.
_PYTEST_RESULT_RE = re.compile(r"^(?:=+\s+)?(.*?\bin\s+[\d.]+s.*?)(?:\s+=+)?$")
_PYTEST_COUNT_RE = re.compile(r"(\d+)\s+(passed|failed|error|errors|skipped|xfailed|xpassed|deselected|warning|warnings)")
_PYTEST_DUR_RE = re.compile(r"\bin\s+([\d.]+)s")
# The FAILURES banner opens the detailed traceback section; the "E   ..."
# lines within carry assertion text.
_PYTEST_FAILURES_BANNER_RE = re.compile(r"^=+\s+FAILURES\s+=+$")
_PYTEST_ERRORS_BANNER_RE = re.compile(r"^=+\s+ERRORS\s+=+$")
_PYTEST_SUMMARY_BANNER_RE = re.compile(r"^=+\s+short test summary info\s+=+$")
_PYTEST_SESSION_START_RE = re.compile(r"^=+\s+test session starts\s+=+$")


class PytestAdapter(OutputAdapter):
    """Adapter for `pytest` output.

    pytest is fundamentally different from go test: it's one session (not
    per-package sections) and its most reliable signals vary by verbosity.
    Across ALL modes two things are dependable:
      - the final result line ("N passed, M failed in Xs"), and
      - the "short test summary info" block on failure
        ("FAILED path::test - <reason>"), which gives a one-line reason.
    In -v mode we additionally get per-test "path::test PASSED/FAILED [NN%]"
    lines. -q mode gives only progress dots, so we lean on the summary block.

    We model each TEST FILE as a section (mirroring go's per-package model,
    so failedSections + drill-down work), attribute each test to its file,
    and attach the summary-info reason as that test's failure detail.
    """
    name = "pytest"

    def __init__(self) -> None:
        # nodeid -> status ("pass"/"fail"/"error"/"skip")
        self._tests: dict[str, str] = {}
        # nodeid -> one-line reason from short-test-summary-info
        self._reasons: dict[str, str] = {}
        # Ordered nodeids as first seen (stable output order)
        self._order: list[str] = []
        self._first_line: dict[str, int] = {}
        # Final counts + duration parsed from the result line
        self._counts: dict[str, int] = {}
        self._duration: float = 0.0
        # State: which detail section we're inside
    ...
    def sniff(self, early_lines: list[str]) -> bool:
        for l in early_lines:
            s = _strip_ansi(l)
            if _PYTEST_SESSION_START_RE.match(s):
                return True
            if s.startswith("platform ") and "pytest" in s:
                return True
            if _PYTEST_VERBOSE_RE.match(s):
                return True
            # -q mode has no banner -- just progress dots + the result line.
            # The result line ("N passed[, M failed] in Xs") is distinctive
            # enough to identify pytest on its own; it appears within the
            # sniff window for short runs.
            m = _PYTEST_RESULT_RE.match(s.rstrip())
            if m and _PYTEST_COUNT_RE.search(m.group(1)):
                return True
        return False

    def _note(self, nodeid: str, status: str, line_no: int) -> None:
        if nodeid not in self._tests:
            self._order.append(nodeid)
            self._first_line[nodeid] = line_no
        # fail/error win over any earlier pass (rerunfailures can re-report)
        prev = self._tests.get(nodeid)
        if prev in ("fail", "error") and status not in ("fail", "error"):
            return
        self._tests[nodeid] = status

    def feed(self, line: str, line_no: int, fallback_at: int) -> list[ParsedEvent]:
        s = _strip_ansi(line).rstrip()

        # Verbose per-test result line.
        m = _PYTEST_VERBOSE_RE.match(s)
        if m:
            nodeid, word = m.group(1), m.group(2)
            status = {
                "PASSED": "pass", "FAILED": "fail", "ERROR": "error",
                "SKIPPED": "skip", "XFAIL": "skip", "XPASS": "pass",
            }[word]
            self._note(nodeid, status, line_no)
            return []

        # short-test-summary-info block: "FAILED path::test - reason"
        if _PYTEST_SUMMARY_BANNER_RE.match(s):
            self._in_summary_block = True
            return []
        if self._in_summary_block:
            ms = _PYTEST_SUMMARY_LINE_RE.match(s)
            if ms:
                word, nodeid, reason = ms.group(1), ms.group(2), ms.group(3)
                status = "fail" if word == "FAILED" else "error"
                self._note(nodeid, status, line_no)
                if reason:
                    self._reasons[nodeid] = reason.strip()
                return []

        # Final result line closes the session. Guard against matching random
        # "... in Ns" log lines: require at least one pytest count word
        # (passed/failed/error/skipped/...) in the same line.
        m = _PYTEST_RESULT_RE.match(s)
        if m:
            body = m.group(1)
            counts = list(_PYTEST_COUNT_RE.finditer(body))
            real_counts = [c for c in counts if c.group(2) not in ("warning", "warnings")]
            if real_counts:
                for cm in counts:
                    n, word = int(cm.group(1)), cm.group(2)
                    key = {"errors": "error", "warnings": "warning"}.get(word, word)
                    self._counts[key] = n
                dm = _PYTEST_DUR_RE.search(body)
                if dm:
                    self._duration = float(dm.group(1))
                return self._emit(line_no, fallback_at)

        return []

    def finalize(self, fallback_at: int) -> list[ParsedEvent]:
        if self._emitted or not self._order and not self._counts:
            return []
        return self._emit(0, fallback_at, incomplete=True)

    def _file_of(self, nodeid: str) -> str:
        # "tests/x/test_foo.py::test_bar[case]" -> "tests/x/test_foo.py"
        return nodeid.split("::", 1)[0]

    def _emit(self, line_no: int, fallback_at: int, *, incomplete: bool = False) -> list[ParsedEvent]:
        self._emitted = True
        out: list[ParsedEvent] = []

        # Group tests by file -> one section per file (mirrors go packages).
        files: dict[str, list[str]] = {}
        for nodeid in self._order:
            files.setdefault(self._file_of(nodeid), []).append(nodeid)

        # If -q gave us no per-test nodeids but we have counts, emit a single
        # synthetic session section so testSummary still carries the totals.
        if not files and self._counts:
            files = {"pytest session": []}

        # Synthetic single-session case (-q with no per-test nodeids): the
        # only truth is the final counts. Failure is derived from those.
        synthetic_session = list(files.keys()) == ["pytest session"] and not self._order
        counts_failed = self._counts.get("failed", 0) + self._counts.get("error", 0)

        for fname, nodeids in files.items():
            f_first = min((self._first_line.get(n, line_no) for n in nodeids), default=max(1, line_no - 1))
            if synthetic_session:
                has_fail = counts_failed > 0
            else:
                has_fail = any(self._tests.get(n) in ("fail", "error") for n in nodeids)
            status = "failed" if has_fail else "ok"

            out.append(ParsedEvent(
                line_no=f_first, at=fallback_at, kind="section_start",
                section=fname,
                payload={"v": "section_start", "name": fname, "synth": "pytest"},
            ))

            fpass = ffail = fskip = 0
            for nodeid in nodeids:
                st = self._tests.get(nodeid, "pass")
                if st == "pass":
                    fpass += 1
                ...
                out.append(ParsedEvent(
                    line_no=self._first_line.get(nodeid, f_first), at=fallback_at,
                    kind="event", section=fname,
                    payload={"v": "event", "msg": f"{st.upper()} {short}", "test": short, "status": st},
                ))
                # Failure reason as failure-detail (the one-liner from the
                # summary block is the highest-signal, cleanest detail).
                if st in ("fail", "error") and nodeid in self._reasons:
                    out.append(ParsedEvent(
                        line_no=self._first_line.get(nodeid, f_first), at=fallback_at,
                        kind="event", section=fname,
                        payload={"v": "event", "msg": self._reasons[nodeid],
                                 "kind": "failure-detail", "test": short},
                    ))

            metric_payload: dict[str, Any] = {"v": "metric", "section": fname}
            if self._duration and (synthetic_session or len(files) == 1):
                metric_payload["elapsedSec"] = self._duration
            # Per-file per-test counts when we saw nodeids; else fall back to
            # the session-level final counts (the -q case).
            if nodeids:
                metric_payload["testsPass"] = fpass
                metric_payload["testsFail"] = ffail
                metric_payload["testsSkip"] = fskip
            elif synthetic_session:
                metric_payload["testsPass"] = self._counts.get("passed", 0)
                metric_payload["testsFail"] = counts_failed
                metric_payload["testsSkip"] = self._counts.get("skipped", 0)
            out.append(ParsedEvent(
                line_no=line_no or f_first, at=fallback_at, kind="metric",
                section=fname, payload=metric_payload,
            ))

            end_payload: dict[str, Any] = {"v": "section_end", "name": fname, "status": status}
            if incomplete:
                end_payload["status"] = "unknown"
                end_payload["reason"] = "no result line (run incomplete)"
            elif has_fail:
                end_payload["reason"] = "test failures"
            out.append(ParsedEvent(
                line_no=line_no or f_first, at=fallback_at, kind="section_end",
                section=fname, payload=end_payload,
            ))
...
_ADAPTERS: list[type[OutputAdapter]] = [GoTestAdapter, PytestAdapter]
...
def _read_last_lines(path: Path, n: int) -> list[str]:
    ...
    try:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as f:
            lines = f.readlines()
        return [_read_log_line(l) for l in lines[-n:]]
    except OSError:
        return []
    ...
    try:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for i, line in enumerate(f):
                if i >= hard_cap:
                    break
                out.append(_read_log_line(line))
        return out
    except OSError:
        return []
...
def _build_test_summary(
    *,
    ...
    """Build the normalized, failure-proportional test summary for an
    adapter-driven run (currently go-test).

    The shape is framework-agnostic on purpose: `framework`, `packages`,
    `tests`, `failures[]`, and `nextCalls`. A future pytest/vitest adapter
    populates the same keys, so the agent-facing contract never changes
    when a new adapter is added -- only the `framework` value differs.

    Green runs collapse to counts + a few slowest packages. Red runs inline
    each failing test's assertion detail (bounded) plus a logRef pointer
    into stdout.log so the agent can drill into the full package output
    without a guess.
    """
    ...
    for sec in sections:
        ...
        elapsed = m.get("elapsedSec")
        if isinstance(elapsed, (int, float)) and elapsed > 0:
            slowest.append({"name": sec.name, "durationSec": round(float(elapsed), 3)})

        # Collect failing tests + their captured assertion-detail lines.
        # build_sections tags failing test events with testStatus=="fail"
        # and failure-detail lines with eventKind=="failure-detail", both
        # carrying the owning test name.
        if sec.status == "failed":
            # Group failure-detail lines by test name for inlining.
            detail_by_test: dict[str, list[str]] = {}
            failing_tests: list[str] = []
            for ev in sec.events:
                if ev.get("eventKind") == "failure-detail" and ev.get("test"):
                    detail_by_test.setdefault(ev["test"], []).append(ev.get("msg", ""))
                elif ev.get("testStatus") == "fail" and ev.get("test"):
                    failing_tests.append(ev["test"])

            log_ref = {
                "section": sec.name,
                "fromLine": sec.line_start,
                "toLine": sec.line_end if sec.line_end is not None else sec.line_start,
            }

            if failing_tests:
                for tname in failing_tests:
                    detail = detail_by_test.get(tname, [])
                    entry: dict[str, Any] = {
                        "package": sec.name,
                        "test": tname,
                        "logRef": log_ref,
                    ...
                    if detail:
                        entry["detail"] = detail[:TEST_FAILURE_DETAIL_CAP]
                        if len(detail) > TEST_FAILURE_DETAIL_CAP:
                            entry["detailTruncated"] = len(detail) - TEST_FAILURE_DETAIL_CAP
                    failures.append(entry)
            else:
                # Package failed without per-test attribution (build failure,
                # or no -v). Surface a package-level failure with its reason.
                entry = {
                    "package": sec.name,
                    "logRef": log_ref,
                }
    ...
    # Slowest packages: descending by duration, small cap. Green-run signal.
    slowest.sort(key=lambda s: s["durationSec"], reverse=True)
    slowest = slowest[:TEST_SLOWEST_PACKAGES]

    # Package counts by ACTUAL section status. An `unknown` section is an
    # in-flight / incomplete unit (a package still running when polled, or a
    # crash before its summary line) -- it must NOT be counted as passed, or
    # a mid-run poll reads as "all green" while work is still happening.
    pkg_passed = sum(1 for s in sections if s.status == "ok")
    pkg_failed = sum(1 for s in sections if s.status == "failed")
    pkg_incomplete = sum(1 for s in sections if s.status not in ("ok", "failed"))
    ...
    if pkg_incomplete:
        packages["incomplete"] = pkg_incomplete
    summary: dict[str, Any] = {
        "framework": parser_used,
        "packages": packages,
    }

    tests_block: dict[str, Any] = {
        "perTestCountsAvailable": per_test_known,
    }
    if per_test_known:
        tests_block["passed"] = total_pass
        tests_block["failed"] = total_fail
        tests_block["skipped"] = total_skip
    else:
        tests_block["hint"] = (
            "Per-test counts unavailable (no -v). Package-level pass/fail is "
            "authoritative; see packages.run / packages.failed."
        )
    summary["tests"] = tests_block

    # Inline failures (bounded), then advertise drill-downs for the rest.
    next_calls: list[dict[str, Any]] = []
    if failures:
        inlined = failures[:TEST_FAILURES_INLINE_CAP]
        summary["failures"] = inlined
        if len(failures) > TEST_FAILURES_INLINE_CAP:
            summary["failuresTruncated"] = len(failures) - TEST_FAILURES_INLINE_CAP
        # One drill-down per distinct failed package (dedup, preserve order).
        ...
        for f in failures:
            ...
            next_calls.append({
                "tool": "runner_section",
                "args": {"runId": run_id, "name": pkg},
                "purpose": f"Full output for failed package {pkg}.",
            })
        summary["nextCalls"] = next_calls

    # Status: distinguish real outcomes so an agent never misreads a clean
    # multi-package run (no -v) as "no tests ran" -- nor a mid-run poll (with
    # in-flight packages) as "all passed".
    if pkg_failed > 0:
        # A failure is authoritative even if other packages are still running.
        summary["status"] = "failed"
    elif pkg_incomplete > 0:
        # Packages still in flight (or crashed pre-summary) and nothing has
        # failed yet: the outcome is not final. Response-level state/result/
        # terminal say whether the process is done; this says the test
        # outcome is still open.
        summary["status"] = "running"
    elif per_test_known and total_pass > 0:
        summary["status"] = "passed"
    elif len(sections) > 0:
        # Packages tested, all ok, no per-test breakdown (typical
        # `go test ./...` without -v). This IS a successful run.
        summary["status"] = "packages_ok"
    else:
        summary["status"] = "no_tests"

    # Slowest packages only on an otherwise-clean run and only when they
    # carry signal (omitted entirely when empty or when there are failures
    # to focus on).
    if not failed_names and slowest:
        summary["slowestPackages"] = slowest
...
def _build_status_response(
    *,
    ...
    # Adapter detection (e.g. go-test). Computed HERE, before the response is
    # assembled, because it gates how much raw structure we surface: for an
    # adapter run the normalized testSummary (built below) is the authoritative
    # view, so the raw per-package `recentlyChangedSections` list and the
    # per-package `delta.newEvents` stream -- which just re-state, package by
    # package, what testSummary already carries -- are suppressed unless the
    # caller asks for verbose. A green 93-package run thus collapses from
    # ~11k tokens of duplicated section events to a compact summary.
    ...
    for sec in sections:
        ...
        if parser_used:
            break
    # For adapter runs the normalized testSummary is the surface; suppress the
    # raw section/event duplication unless verbose. Non-adapter runs keep the
    # delta stream (it's their only structured progress signal).
    suppress_raw_for_adapter = bool(parser_used) and not verbose

    # recently changed sections: those whose section_end appeared after cursor.
    # For adapter runs, only surface sections that FAILED (passing packages are
    # already counted in testSummary.packages); the full list is verbose-only.
    recently_changed = []
    if not suppress_raw_for_adapter:
        for sec in sections:
        ...
        # Adapter run, non-verbose: keep only failed sections here so a red
        # run still shows what changed without the all-green package dump.
        # NOTE: only FAILED sections here. A streaming adapter (pytest) that
        # hasn't seen its result line yet closes every in-flight file as
        # `unknown` on each mid-run poll -- that's a polling artifact, not a
        # signal, and including it dumped the entire suite (47 files x3) into
        # a running-status response. The real "did it crash" signal for
        # adapter runs lives in testSummary.status + result + exitCode.
        for sec in sections:
            if sec.line_end is not None and sec.line_end > cursor_line and sec.status == "failed":
                entry = {
                    "name": sec.name,
                    "status": sec.status,
                    "durationSec": sec.duration_sec,
    ...
    # produce one event per test AND a section_start/metric/section_end triplet
    # per package. We compact this for the agent:
    #
    #   - For adapter runs (non-verbose), suppress the ok/passing section
    #     boundary + metric events entirely -- testSummary carries the totals.
    #     Only failing-test events and failed-section boundaries survive.
    #   - Always suppress passing/skipping per-test events (kept: failing
    #     tests, failure-detail lines).
    #   - Aggregate suppressed counts into delta.suppressed* so the agent
    #     knows what was hidden (and can ask for verbose if needed).
    ...
    suppressed_section_events = 0
    suppressed_detail_events = 0
    truncated_events = 0
    for ev in new_events:
        ...
        if not verbose and ev.kind == "event" and ev.payload.get("status") in ("pass", "skip") and ev.payload.get("test"):
            if ev.payload.get("status") == "pass":
                ...
                suppressed_skip += 1
            continue
        # For adapter runs (non-verbose), drop the raw failure-DETAIL lines
        # (assertion text) from the delta stream: testSummary.failures[] now
        # carries them cleaned + attributed per test, so streaming each
        # Error/Error Trace/expected/actual line here is pure duplication.
        # The `FAIL <test>` summary events are kept (a cheap at-a-glance
        # signal); only the multi-line detail is elided.
        if suppress_raw_for_adapter and ev.kind == "event" and ev.payload.get("kind") == "failure-detail":
            suppressed_detail_events += 1
            continue
        # For adapter runs (non-verbose), drop ok/section-boundary + metric
        # events -- they only re-state testSummary. Keep ONLY failed-section
        # boundaries. `unknown` section_ends are suppressed too: a streaming
        # adapter (pytest) re-closes every in-flight file as `unknown` on each
        # mid-run poll, which would otherwise flood the delta with one event
        # per test file on every poll of a running suite.
        if suppress_raw_for_adapter:
            if ev.kind == "metric":
                suppressed_section_events += 1
                continue
            if ev.kind in ("section_start", "section_end"):
                status_val = ev.payload.get("status")
                if ev.kind == "section_start" or status_val != "failed":
                    suppressed_section_events += 1
                    continue
        entry: dict[str, Any] = {
            "kind": ev.kind,
    ...
    response: dict[str, Any] = {
        ...
        "delta": {
            ...
            **({"suppressedSectionEvents": suppressed_section_events,
                "suppressedSectionHint": "Passing-package section/metric events hidden; testSummary carries the totals. Pass verbose:true for the raw per-package stream."}
               if suppressed_section_events > 0 else {}),
            **({"suppressedDetailEvents": suppressed_detail_events,
                "suppressedDetailHint": "Raw assertion-detail lines hidden; the same detail is inlined + attributed per test in testSummary.failures[]. Pass verbose:true for the raw stream."}
               if suppressed_detail_events > 0 else {}),
    ...
    # recentlyChangedSections: only when non-empty. For adapter runs this is
    # now failed-sections-only (non-verbose), so a green run omits it and a
    # red run highlights exactly the packages that changed to a bad state.
    if recently_changed:
        response["recentlyChangedSections"] = recently_changed
    if fatal_msg:
    ...
    # unknownSections is a real signal for an INSTRUMENTED (non-adapter) run:
    # a section still open at exit means a crashed script. For a streaming
    # ADAPTER run (pytest) it's a polling artifact -- every in-flight test
    # file reads as `unknown` until the final result line -- so listing all of
    # them (the whole suite, on every mid-run poll) is pure noise. The genuine
    # crash signal for adapter runs is testSummary.status + result + exitCode.
    if unknown_names and not parser_used:
    ...
    # Adapter-driven test summary. `parser_used` was detected earlier (it
    # gates the raw-section suppression above). When set, build the
    # normalized, failure-proportional testSummary -- the authoritative
    # surface for adapter runs. The raw per-package stream lives on disk for
    # runner_section / runner_grep.
    if parser_used:
        response["parserUsed"] = parser_used
        response["testSummary"] = _build_test_summary(
            run_id=meta["runId"],
            sections=sections,
    ...
    #
    # Adapter-failure skip: when an adapter (go-test) already produced a
    # structured testSummary.failures[] -- each failing test with its
    # assertion detail inlined -- the warning scan just re-surfaces those
    # same "FAIL"/"Error:" lines raw and tab-escaped. That's pure
    # duplication of what the agent already has cleanly. We still scan
    # adapter runs that exited GREEN (the "don't trust a green result"
    # case: error-shaped lines with a 0 exit still warrant a heads-up).
    adapter_failure = bool(parser_used) and bool(failed_names)
    if terminal and not is_agent_run and not adapter_failure:
    ...
    try:
        ...
        with path.open("r", encoding="utf-8", errors="replace") as f:
            for i, line in enumerate(f, start=1):
                ...
                if i > end:
                    break
                lines.append(_read_log_line(line))
...
def _list_cache_key(d: Path) -> dict[str, Any]:
    """Fingerprint of every input cmd_list derives a scoreboard entry from.
    Any append to the logs or rewrite of meta.json changes size/mtime and
    invalidates the cached entry."""
    key: dict[str, Any] = {"v": _LIST_CACHE_VERSION}
    for label, name in (("meta", FILE_META), ("stdout", FILE_STDOUT), ("stderr", FILE_STDERR)):
        try:
            st = (d / name).stat()
            key[label] = [st.st_size, st.st_mtime_ns]
        except OSError:
            key[label] = None
    return key


def _load_list_entry_cache(d: Path) -> dict[str, Any] | None:
    """Return the cached scoreboard entry for a run, or None when absent or
    stale. Only terminal runs are ever cached (see cmd_list), so a hit means
    the expensive parse/synthesize/extract work can be skipped entirely."""
    cached = _read_json(d / FILE_LIST_CACHE)
    if not isinstance(cached, dict) or cached.get("key") != _list_cache_key(d):
        return None
    entry = cached.get("entry")
    return entry if isinstance(entry, dict) else None


def _save_list_entry_cache(d: Path, entry: dict[str, Any]) -> None:
    """Best-effort persist; a failed cache write must never break listing."""
    try:
        _atomic_write_json(d / FILE_LIST_CACHE, {"key": _list_cache_key(d), "entry": entry})
    except OSError:
        pass


def _stream_idle_sec(d: Path, now: int) -> int | None:
    """Seconds since EITHER output stream last changed.

    idle = now - max(mtime(stdout.log), mtime(stderr.log)).

    Using the max (most-recent) of the two is deliberate: an agent turn can
    go quiet on stdout for a long time while still healthy -- a long tool
    call (build/test/deploy) or model thinking produces raw NDJSON on
    stderr with nothing rendered to stdout yet. The render pipeline mirrors
    every opencode event to stderr, so stderr keeps advancing as long as the
    backend emits ANYTHING. Only when BOTH streams have gone silent has the
    backend genuinely stopped producing output. Returns None when neither
    log exists (can't judge)."""
    latest: int | None = None
    for name in (FILE_STDOUT, FILE_STDERR):
        try:
            m = int((d / name).stat().st_mtime)
        except OSError:
            continue
...
def _reconcile_running_meta(d: Path, meta: dict[str, Any]) -> dict[str, Any]:
    """If meta says 'running' but the PID is gone, persist the crash to disk.

    cmd_status self-heals a dead-pid run by writing state=exited back to
    meta.json (see _block/_status paths). cmd_list must do the same, or a
    run whose process died without runner updating meta (crash, kill -9,
    reboot) shows 'running' forever in the scoreboard. We rewrite meta.json
    here so the fix is durable and the (meta-fingerprinted) list cache
    invalidates on the next call. Returns the (possibly updated) meta dict.
    Best-effort: a failed write leaves the in-memory correction for this
    call and retries next time."""
    ...
    if meta.get("exitCode") is None:
        meta["exitCode"] = -1   # unknown: runner never recorded a real code
    if meta.get("endedAt") is None:
        try:
            meta["endedAt"] = int((d / FILE_STDOUT).stat().st_mtime)
        except OSError:
            meta["endedAt"] = int(time.time())
    try:
        _atomic_write_json(d / FILE_META, meta)
    except OSError:
        pass
    return meta
...
def cmd_list(args: argparse.Namespace) -> int:
    ...
    for d in run_dirs:
        ...
        if not meta:
            continue
        # Self-heal first: if meta says 'running' but the PID is gone, persist
        # state=exited to meta.json NOW (parity with cmd_status). This must
        # happen BEFORE the cache read -- rewriting meta.json bumps its mtime
        # and invalidates any stale 'running' cache entry, so a crashed run
        # can never stay 'running' in the scoreboard.
        meta = _reconcile_running_meta(d, meta)

        # Terminal runs are immutable: their scoreboard entry is cached in
        # the run dir after first computation so history-heavy projects
        # don't re-parse every old log on every list call. The cache is
        # fingerprinted on meta/stdout/stderr size+mtime, so any change
        # falls through to the full computation below.
        cached_entry = _load_list_entry_cache(d)
        if cached_entry is not None:
            entry = dict(cached_entry)
            entry["runRoot"] = str(d)
            # lastLineAgeSec drifts with wall time; recompute from mtime
            if "lastLine" in entry:
                try:
                    entry["lastLineAgeSec"] = max(0, now - int((d / FILE_STDOUT).stat().st_mtime))
                except OSError:
                    pass
            runs.append(entry)
            continue
        ...
        # Stall detection for LIVE agent turns. The process is alive (else
        # _reconcile_running_meta would have flipped it to exited above), but
        # if BOTH output streams have gone silent past AGENT_STALL_SEC the
        # backend stream almost certainly died mid-turn (opencode ends the
        # turn without a reason=stop / error event, leaving the process idle
        # in its event loop). Surface stalled=true + idleSec so the run stops
        # masquerading as a healthy 'running' job. Non-destructive: we never
        # kill it here; the operator/agent decides (runner_kill to reap).
        if synth["state"] == "running" and meta.get("agentRuntime"):
            idle = _stream_idle_sec(d, now)
            if idle is not None and idle >= AGENT_STALL_SEC:
                entry["stalled"] = True
                entry["idleSec"] = idle
                entry.setdefault("agent", {})["stalled"] = True
                entry["agent"]["idleSec"] = idle

        runs.append(entry)
        # Only terminal runs are safe to cache: nothing appends to their
        # logs anymore, so the entry can be reused until the files change.
        if synth["state"] == "exited":
            _save_list_entry_cache(d, entry)
...
def _run_grep(
    ...
    *,
    ...
    for stream_name, path in streams:
        ...
        try:
            with path.open("r", encoding="utf-8", errors="replace") as f:
                lines = [_read_log_line(l) for l in f.readlines()]
        except OSError:
            continue
...
def cmd_wait_for(args: argparse.Namespace) -> int:
    ...
    if outcome == "matched":
        ...
        if endpoints:
            response["endpoints"] = endpoints
        # A match means the PATTERN appeared -- NOT that the run finished or
        # succeeded. An early banner line (e.g. "starting up") matches a loose
        # pattern while the process is still working, and a matched wait says
        # nothing about the eventual exit code. The only authoritative outcome
        # is runner_status. Agents have mistaken an early match (and an
        # aborted/interrupted wait) for a completed or failed run.
        response["note"] = (
            "Pattern matched -- this does NOT mean the run finished or "
            "succeeded, only that the line appeared. For the authoritative "
            "state/result/exitCode, call runner_status (wait=false). If you "
            "were waiting for readiness, tighten the pattern to the true "
            "ready line, not an early banner."
        )
    elif outcome == "exited":
        meta = _read_json(rdir / FILE_META, default={}) or {}
        ...
        if meta.get("agentRuntime"):
            ...
            response["followUp"] = (
                ...
                f"(NOT killed by this wait). IMPORTANT: this timeout -- and any "
                f"'aborted'/'interrupted' notice on a wait call -- is NOT "
                f"authoritative about the run. runner NEVER kills the process "
                f"on a wait; it is detached and keeps going. Call runner_status "
                f"(wait=false) for the real state/result/exitCode: it may still "
                f"be running, have finished successfully, or have failed -- "
                f"verify, do not assume. Then runner_wait_for again to keep "
                f"waiting, or runner_kill to actually stop it."
            )
...
def _resolve_session_adoption(
    agents: Any,
    session_id: str,
    requested_runtime: str | None,
    cwd: str,
) -> dict[str, Any]:
    """Decide which backend runtime should adopt `session_id`.

    Called when --ask + --run-id is set but the id resolved to no runner
    run. Probes runtimes (only the requested one when --agent was
    explicit) via their optional hooks:

      looks_like_session_id(s): shape claim (opencode 'ses_...',
          claude/codex UUID). A runtime that doesn't claim the shape is
          skipped -- so e.g. an explicit --agent opencode with a UUID is
          refused up front instead of spawning a doomed turn.
      find_session(s, cwd): on-disk verification (claude/codex). When the
          session belongs to a DIFFERENT project, the returned
          projectDir tells the caller to spawn the backend in that
          directory so a resumed agent turn retains its original project
          context. The run dir itself stays in the ACTIVE
          project's run root so the calling agent can still see and
          poll the run.

    Returns {"runtime": name, "projectDir": str|None} on success
    (projectDir set only when the backend must run in another
    directory), or {"error": ..., ...} when nothing claims the id.
    """
    names = [requested_runtime] if requested_runtime else agents.names()
    unvalidated: str | None = None
    ...
    for name in names:
        ...
        if rt is None:
            continue
        looks = getattr(rt, "looks_like_session_id", None)
        if looks is not None and not looks(session_id):
            detail.append(f"{name}: id shape not recognized")
            continue
        finder = getattr(rt, "find_session", None)
        if finder is None:
            # Shape claimed but no local existence check (opencode):
            # remember as fallback; a verified match from another
            # runtime wins over an unverified one.
            if unvalidated is None:
                unvalidated = name
            continue
        try:
            info = finder(session_id, cwd) or None
        except Exception:
            info = None
        if info is None:
            detail.append(f"{name}: no session transcript on disk")
            continue
        project_dir = info.get("projectDir")
        verified.append({
            "runtime": name,
            "projectDir": None if info.get("sameProject") else project_dir,
        })
    if len(verified) == 1:
        return verified[0]
    if len(verified) > 1:
        matches = [match["runtime"] for match in verified]
        return {
            "error": (
                f"session id {session_id!r} exists in more than one "
                f"agent backend: {', '.join(matches)}"
            ),
            "matches": matches,
            "hint": "repeat the request with an explicit agent value",
        }
    if unvalidated is not None:
        return {"runtime": unvalidated, "projectDir": None}
    return {
        "error": (
            f"{session_id!r} is neither a known runner runId nor a "
            f"backend session id that any agent runtime recognizes"
        ),
        "detail": detail or None,
        "hint": (
            "runner runIds come from a previous "
            "research_and_code_assistant_agent response (see "
            "runner_list); opencode session ids look like 'ses_...'; "
            "claude and codex session ids are UUIDs and must have a "
            "transcript under ~/.claude/projects/ or "
...
def cmd_agent(args: argparse.Namespace) -> int:
    ...
    Four modes, distinguished by which of --ask / --run-id are set:
      ...
      4. POLL FOR RESPONSE (--run-id, NO --ask):
         ...
         the existing runner run, then blocks until the turn reaches a
         final reply (or the wait window elapses, or the process exits).
    ...
    if agents is None:
        ...
        return 1
    # The runtime the caller EXPLICITLY asked for (None when omitted).
    # Kept separate from the effective runtime_name because a
    # continuation inherits the run's own backend from meta.agentRuntime
    # -- callers shouldn't need to repeat `agent` on every turn.
    requested_runtime = (getattr(args, "agent", None) or "").strip() or None
    runtime_name = requested_runtime or "opencode"
    runtime = agents.get(runtime_name)
    if runtime is None:
    ...
    adopt_cwd: str | None = None
    raw_run_id = getattr(args, "run_id", None)
    if raw_run_id:
        if _is_runner_run_id(raw_run_id):
            # Mode 2: continuation. Runner runIds take precedence, but a
            # miss is NOT fatal any more: claude/codex session ids are plain
            # UUIDs and can even be v7-shaped (callers may pre-assign
            # them via `claude --session-id`), so an id that resolves to
            # no run falls through to backend-session adoption below.
            resolve_ns = argparse.Namespace(run_id=raw_run_id, cwd=getattr(args, "cwd", None))
            rdir = _resolve_run_dir(resolve_ns, quiet=True)
        if rdir is not None:
            meta = _read_json(rdir / FILE_META, default={}) or {}
            if meta.get("agentRuntime"):
                if requested_runtime and meta["agentRuntime"] != requested_runtime:
                    print(json.dumps({
                        "error": f"run {raw_run_id} is a {meta['agentRuntime']} conversation, "
                                 f"cannot continue with {requested_runtime}",
                    }), file=sys.stderr)
                    return 1
                # No explicit --agent (or it matches): continue on the
                # backend the conversation started with.
                runtime_name = meta["agentRuntime"]
                runtime = agents.get(runtime_name)
                if runtime is None:
                    available = agents.names() if hasattr(agents, "names") else []
                    print(json.dumps({
                        "error": f"run {raw_run_id} uses agent runtime "
                                 f"{runtime_name!r}, which is not available",
                        "available": available,
                    }), file=sys.stderr)
                    return 1
        else:
            # Mode 3: adoption. Treat raw_run_id as a backend session id
            # and ask the runtimes which one recognizes it.
            resolved = _resolve_session_adoption(
                agents, raw_run_id, requested_runtime,
                getattr(args, "cwd", None) or os.getcwd())
            if "error" in resolved:
                print(json.dumps(resolved), file=sys.stderr)
                return 1
            runtime_name = resolved["runtime"]
            runtime = agents.get(runtime_name)
            # Cross-project backend session: the process runs in the
            # session's own project dir so resumed turns retain their
            # original context. The run dir stays under the ACTIVE
            # project's run_root so this agent can still see and poll it.
            adopt_cwd = resolved.get("projectDir")
            # Check if runner already wraps it; if so, redirect.
    ...
    # Resolve the model for this turn. An explicit --model always wins and
    # is validated against the backend's real catalog; on a continuation
    # with no new --model, we reuse the model persisted from the turn that
    # first set it (meta.agentModel), which was already validated when it
    # was introduced. No model anywhere => opencode's configured default.
    #
    # Model switching is operator-driven, not agent-driven: the dispatch
    # tool only forwards a model the caller explicitly passed. When an
    # invalid one comes through we refuse the turn (before any spawn) and
    # return the exact valid list so the caller can correct it.
    requested_model = getattr(args, "model", None)
    if requested_model is not None:
        requested_model = requested_model.strip() or None
    if requested_model is not None and hasattr(runtime, "validate_model"):
        is_valid, valid_models = runtime.validate_model(requested_model)
        if not is_valid:
            model_list = "\n".join(valid_models) if valid_models else "(none found)"
            print(json.dumps({
                "error": (
                    "Invalid model, only specify a model if the operator has "
                    "requested specific tasks to use a model other than the "
                    "default. These are the valid models to choose from: \n"
                    f"{model_list}"
                ),
                "invalidModel": requested_model,
                "validModels": valid_models,
            }), file=sys.stderr)
            return 2
    model_for_turn = requested_model or (meta.get("agentModel") if rdir is not None else None)

    # Write the new turn's prompt. All current backends consume it via stdin
    ...
    cmd_to_run = runtime.build_cmd(prompt_path, backend_session_id, model_for_turn)
    started_at = int(time.time())
    ...
    new_meta: dict[str, Any] = {
        ...
        "cmd": cmd_to_run,
        # adopt_cwd (cross-project backend adoption) outranks everything:
        # the backend must run in the session's own project dir. It is
        # persisted here so later turns of the adopted conversation
        # inherit it via meta.get("cwd").
        "cwd": (adopt_cwd or getattr(args, "cwd", None) or meta.get("cwd") or os.getcwd()),
        "startedAt": started_at,
        "endedAt": None,
        "exitCode": None,
        "state": "starting",
        ...
        "agentModel": model_for_turn,           # None => backend default
    ...
    start_payload: dict[str, Any] = {
        ...
        "agent": {
            ...
            "model": model_for_turn,   # None => backend default
            # Where the sub-agent process actually runs. Differs from
            # the caller's cwd when adopting a backend session that
            # belongs to another project -- the run itself still lives
            # in the active project's run root, so it stays visible to
            # this agent.
            "cwd": new_meta["cwd"],
        },
    }
...
def cmd_models(args: argparse.Namespace) -> int:
    """List the models each agent backend accepts for --model.

    One command covering every registered runtime (or a single one via
    --agent). Each backend's optional available_models() supplies
    [{"model": id, "description"?: str}, ...]; its optional MODELS_NOTE
    explains how binding the list is (opencode/codex catalogs are
    authoritative, claude's is advisory pass-through). A backend whose
    lister is missing or returns nothing gets models: [] plus a hint --
    usually the backend CLI isn't installed.
    """
    agents = _agents_module()
    if agents is None:
        print(json.dumps({
        ...
        return 1
    requested = (getattr(args, "agent", None) or "").strip() or None
    names = [requested] if requested else agents.names()
    if requested and agents.get(requested) is None:
        print(json.dumps({
            "error": f"unknown agent runtime: {requested!r}",
            "available": agents.names(),
        }), file=sys.stderr)
        return 2
    response: dict[str, Any] = {"agents": {}}
    for name in names:
        runtime = agents.get(name)
        entry: dict[str, Any] = {"models": [], "count": 0}
        note = getattr(runtime, "MODELS_NOTE", None)
        if note:
            entry["note"] = note
        if hasattr(runtime, "available_models"):
            try:
                models = runtime.available_models() or []
            except Exception as e:
                models = []
                entry["error"] = f"model listing failed: {e}"
            entry["models"] = models
            entry["count"] = len(models)
            if not models and "error" not in entry:
                entry["hint"] = (
                    f"no models found -- is the {name} CLI installed and "
                    f"authenticated on this machine?"
                )
        else:
            entry["hint"] = "this backend does not support model listing"
        response["agents"][name] = entry
    print(json.dumps(response, indent=2 if getattr(args, "pretty", False) else None))
    return 0
...
def _resolve_run_dir(args: argparse.Namespace, quiet: bool = False) -> Path | None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    # No argparse default: cmd_agent must distinguish "omitted" (inherit
    # the conversation's backend on continuation, opencode for new
    # conversations) from an explicit choice (mismatch-checked).
    p.add_argument("--agent", default=None,
                   help="agent runtime name (default: opencode for new "
                        "conversations; continuations inherit the "
                        "conversation's backend)")
    p.add_argument("--model",
                   help="optional provider/model override (e.g. anthropic/claude-fable-5). "
                        "Omit to use the backend's default. Only supply when the operator "
                        "explicitly requests a specific model; invalid values are refused.")
    p.add_argument("--ask",
    ...
    # models (what each agent backend accepts for --model)
    p = sub.add_parser(
        "models",
        help="list the models each agent backend accepts for --model",
    )
    p.add_argument("--agent", default=None,
                   help="only list models for this backend (default: all)")
    p.add_argument("--pretty", action="store_true", help="pretty-print JSON")
